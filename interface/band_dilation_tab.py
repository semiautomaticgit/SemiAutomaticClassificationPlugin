...
This tool allows for the geometric dilation of pixel values.
"""
...
def check_value_list():
    try:
        values = cfg.utils.text_to_value_list(
            cfg.dialog.ui.dilation_classes_lineEdit.text()
        )
        cfg.dialog.ui.dilation_classes_lineEdit.setStyleSheet('color : green')
    except Exception as err:
        str(err)
        cfg.dialog.ui.dilation_classes_lineEdit.setStyleSheet('color : red')
        values = []
    return values
...
# band dilation
def band_dilation():
    bandset_number = cfg.dialog.ui.band_set_comb_spinBox_16.value()
    ...
    if len(values) > 0:
        ...
        if output_path is not False:
            cfg.logger.log.info('band_dilation: %s' % output_path)
            cfg.logger.log.debug('bandset_number: %s' % bandset_number)
            output_name = cfg.dialog.ui.output_name_lineEdit_2.text()
            size = cfg.dialog.ui.dilation_threshold_spinBox.value()
            if cfg.dialog.ui.virtual_output_checkBox_1.isChecked():
                virtual_output = True
            else:
                virtual_output = False
            if cfg.dialog.ui.circular_structure_checkBox_2.isChecked():
                circular = True
            else:
                circular = False
            cfg.ui_utils.add_progress_bar()
            output = cfg.rs.band_dilation(
...
def set_script():
    ...
    command = ('# band dilation (input files from bandset)\n'
               'rs.band_dilation(input_bands=%s, value_list=%s, size=%s, '
